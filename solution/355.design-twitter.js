...
Twitter.prototype.postTweet = function (userId, tweetId) {
  const postTime = this.lastPostID++
...
Twitter.prototype.getNewsFeed = function (userId) {
  const followees = this.followMap[userId] ? [...this.followMap[userId]] : []
  const userIDs = [...new Set(followees.concat([userId]))]
  let tweets = []
  for (let i = 0; i < userIDs.length; i++) {
    const userID = userIDs[i]
    if (this.postMap.has(userID)) {
      tweets.push(...this.postMap.get(userID).slice(0, 10))
    }
  }
  tweets.sort((a, b) => b.postTime - a.postTime)
  return tweets.slice(0, 10).map((v) => v.tweetId)
}
